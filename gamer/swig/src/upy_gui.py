...
class GamerUI(uiadaptor):
    def setup(self, *args, **kwargs):
        ...
        self.domains = {}        
        self.tetmesh_options=["DOLFIN", "FEtk", "Diffpack", "Carp"]
        self.tetmesh_formats=["dolfin", "mcsf", "diffpack", "carp"]
        self.tetmesh_suffices=[".xml", ".m", ".grid", ""]
        self.tetparams = {} 
        self.buttons = {}
        self.labels = {}
    ...
    def initWidget(self,id=None):
        ...
        self.mesh_params["surface_mesh_menu"] = self._addElemt(\
            name="Surface_mesh:", action=self._update_mesh_import,\
       ...
       # Declare UI elements for repair
        ...
        self.tetparams["volume_constraint"] = self._add_float(\
            "Vol constr:", mini=.00001, maxi=100,\
            action=self._update_domain, init=10.,\
            tooltip="The volume of a domain", step=0.001, precision=.001,\
            width=self._width)

        self.tetparams["domain_as_hole"] = self._add_check_box(\
            "Use domain as a hole", action=self._update_domain,
            tooltip="Declare a domain as a hole within a tetrahedral mesh",
            width=self._width)

        self.tetparams["higher_order"] = self._add_check_box(\
            "Higher order mesh generation", action=self._check_higher_order,
            tooltip="Generate higher order mesh",
            width=self._width)

        # Tetmesh formats
        # ---------------------------------------------------------------
        self.tetparams["dolfin_format"] = self._add_check_box(\
            "DOLFIN", tooltip="Write tetmesh in dolfin format",
            width=self._width, init=1)

        self.tetparams["mcsf_format"] = self._add_check_box(\
            "FEtk", tooltip="Write tetmesh in FEtk format",
            width=self._width)

        self.tetparams["carp_format"] = self._add_check_box(\
            "Carp", tooltip="Write tetmesh in Carp format",
            width=self._width)

        self.tetparams["diffpack_format"] = self._add_check_box(\
            "Diffpack", tooltip="Write tetmesh in diffpack format",
            width=self._width)
    ...
    def mesh_improvement_frame(self):
        ...
        frame_info.append([self.labels["empty"]])
        frame_info.append([self.buttons["normal_smooth"], self.labels["empty"],\
                           self.labels["empty"]])
        frame_info.append([self.labels["empty"]])
...
#                           self.buttons["unhide_boundary"]])
    ...
    def tetrahedralization_frame(self, domain=None):
        ...
        if domain:
            ...
            frame_info.append([self.tetparams["dolfin_format"],
                               self.tetparams["diffpack_format"]])
            frame_info.append([self.tetparams["carp_format"],
                               self.tetparams["mcsf_format"]])
            frame_info.append([self.buttons["tetrahedralize"],
                               self.tetparams["higher_order"]])#,\
    ...
    def calculate_areas(self):
        ...
        dump(areas, open(self.helper.getName(obj)+".cpickle", "w"))
...
#        bmesh.faces.extend(add_faces)
    ...
    def centralize(self):
        ...
        self.helper.updateMesh(obj, vertices, smooth=False)
        self.helper.setTranslationObj(obj, [0, 0, 0])
    ...
    def triangulate_holes(self):
        ...
        for edge_loop in edge_loops:
            ...
            for ind, vertex in enumerate(edge_loop):
                coordinates[ind,:] = bmesh.verts[vertex].co
    ...
    def tetrahedralize(self):
        ...
        if not (self.getVal(self.tetparams["dolfin_format"]) or \
                self.getVal(self.tetparams["diffpack_format"]) or \
                self.getVal(self.tetparams["carp_format"]) or \
                self.getVal(self.tetparams["mcsf_format"])):
            self.drawError(errormsg="Select at least one mesh format.")
            return
    ...
    def gamer_to_host(self, gmesh, boundaries, mesh_name="gamer_improved",
        ...
        if self.getVal(self.gparams["create_new_mesh"]):
            ...
            obj, bmesh = self.helper.createsNmesh(mesh_name, verts, None, \
                                                  faces, smooth=0, color=[0.8,0.8,0.8])

            # Select the generated mesh
            self.helper.setCurrentSelection(obj)
            ...
            # Update present mesh
            self.helper.updateMesh(obj, vertices=verts, faces=faces, smooth=False)
